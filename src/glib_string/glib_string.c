...
int main(int argc, char **argv) {
    ...
    g_print("g_string_new(\"Justin\"); len = %d, allocated_len = %d\n",
            (int)string->len, (int)string->allocated_len);
    ...
    g_print("g_string_sized_new(32); len = %d, allocated_len = %d\n",
            (int)string->len, (int)string->allocated_len);
    g_string_free(string, TRUE);
    ...
    //    void g_string_print(GString *string, const gchar *format, ...);  格式化一个字符串，和sprintf用法一样，只是赋值给一个GString，string里面以前的数据被销毁了
